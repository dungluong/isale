...
@Component({
    selector: 'fb-auto-order',
    templateUrl: 'fb-auto-order.component.html'
})
export class FbAutoOrderComponent {
    ...
    constructor(
        ...
        this.navCtrl.unsubscribe('reloadFbAutoOrderConfigs', this.reload);
        this.navCtrl.subscribe('reloadFbAutoOrderConfigs', this.reload);
    }

    async ionViewWillEnter() {
        await this.analyticsService.setCurrentScreen('fb-auto-order-config');
    }
    ...
    reload = async () => {
        ...
        this.omniChannelService.getAutoOrderConfigs().then(async (configs) => {
    ...
    edit(id): void {
        this.navCtrl.push('/fbpage/auto-order-add', { id });
    }

    async search(): Promise<void> {
        this.analyticsService.logEvent('fb-auto-order-search');
    ...
    async updateActive(config) {
        await this.omniChannelService.saveAutoOrderConfig(config);
    }
}
